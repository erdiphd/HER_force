...
class RobotEnv(gym.GoalEnv):
    def __init__(self, model_path, initial_qpos, n_actions, n_substeps, task):
        ...
        self.sum_touch_replay_buffer = self.touch_replay_buffer

        self.success_log = 0.0
    ...
    def step(self, action):
        ...
        ## TODO check_erdi
        self.touch_replay_buffer = np.sum(np.array(np.abs(force_left) + np.abs(force_right)), axis=0) / self.sim.nsubsteps
        self.sum_touch_replay_buffer += self.touch_replay_buffer
        ...
        self.success_log = info['is_success']
        done = False  # if not info['is_success'] else True
    ...
    def reset(self):
        ...
        # reset force measurements
        self.sum_energy *= 0
        self.sum_force *= 0
        self.sum_force_replay_buffer *= 0
        self.sum_touch_replay_buffer *= 0
