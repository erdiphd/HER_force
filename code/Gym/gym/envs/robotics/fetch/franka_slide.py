...
MODEL_XML_PATH = os.path.join('fetch', 'frankaslide.xml')


class FrankaSlideEnv(franka_fetch_env.FetchEnv, utils.EzPickle):
    def __init__(self, reward_type='sparse'):
        ...
        franka_fetch_env.FetchEnv.__init__(
            ...
            gripper_extra_height=-0.02, target_in_the_air=False, target_offset=np.array([0.0, 0.0, 0.0]),
            obj_range=0.1, target_range=0.3, distance_threshold=0.05,
            initial_qpos=initial_qpos, force_range=3, force_reward_weight=0.25, task='frankaslide', reward_type=reward_type)
